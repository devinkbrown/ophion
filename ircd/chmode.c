...
/* Compact registration table for built-in channel modes.
 * Extensions and modules may add further entries via cflag_add() or
 * by writing directly into chmode_table[c] before calling
 * construct_cflags_strings().
 */
static const struct
{
	...
	/* param-list modes (ban / exception / invex / quiet) */
	{'b', chm_ban,      CHFL_BAN      },
	{'e', chm_ban,      CHFL_EXCEPTION },
	{'I', chm_ban,      CHFL_INVEX    },
	{'Z', chm_ban,      CHFL_QUIET    },
	/* membership modes */
	{'o', chm_op,       0             },
	{'q', chm_admin,    0             },
	{'v', chm_voice,    0             },
 ...
 * chmode_init
 *
 * Fills chmode_table with chm_nosuch for all 256 slots, then registers
 * every built-in mode from chmode_builtin[].  Called once at startup
 * (ircd.c) before construct_cflags_strings().
 *
 * Extension modules may call cflag_add() / cflag_orphan() afterwards,
 * or write directly into chmode_table[c], to add or remove modes.
 */
void
chmode_init(void)
{
	...
	for(i = 0; i < 256; i++)
	{
		...
		chmode_table[i].mode_type = 0;
	}

	for(i = 0; chmode_builtin[i].c != 0; i++)
	{
		chmode_table[chmode_builtin[i].c].set_func  = chmode_builtin[i].fn;
		chmode_table[chmode_builtin[i].c].mode_type = chmode_builtin[i].mode_type;
	}
}
